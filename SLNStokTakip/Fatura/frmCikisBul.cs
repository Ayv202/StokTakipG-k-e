    ...
    public partial class frmCikisBul : Form
    {
        ...
        void Listele()
        {
            ...
            var lst = (from s in _db.stUrunCikis
                       where s.FaturaDurum == false //faturası basılmamış değerler gelecek
                       select new {
                           cn=s.CikisNo,
                           fa=s.bgFirma.Fadi,
                           ct=s.Cturu,
                           trh=s.Tarih,
                           fd=s.FaturaDurum
                       }).ToList().Distinct(); //çıkış no için distinct uygulandı
            //new yapmadığımızda idye göre getiriyordu, bu halde liste çıkış noya göre geliyor ve aynı çıkış nolu kayıtların
            //hepsi gelmiyor
